...
export const fetchActiveDailyPrompts =
  async (): Promise<ACTIVE_DAILY_PROMPTS_QUERYResult> => {
    console.log("📡 [API] Fetching active daily prompts...");
    try {
      const prompts = await sanityClient.fetch(ACTIVE_DAILY_PROMPTS_QUERY);
      console.log("✅ [API] Daily prompts fetched:", prompts.length, "prompts");
      return prompts;
    } catch (error) {
      console.error("❌ [API] Error fetching daily prompts:", error);
      throw error;
    }
  };
...
export const getRandomDailyPrompts = async (
  ...
  console.log(`🎲 [API] Getting ${count} random daily prompts...`);
  try {
    ...
    console.error("❌ [API] Error getting random daily prompts:", error);
    throw error;
  }
};
